...
void set_mesh_positions(Domain* domain)
{
  ...
  for (int i=0; i<wet_nodes_size; i++) {
    printf("initial_coords[%d] = (%f, %f)\n", i, initial_coords[counter],
            initial_coords[counter+1]);
    counter += 2;
  }
      ...
      f_node_loop (face, face_thread, n){
        ...
        if (NODE_POS_NEED_UPDATE(node)){
          ...
          for (dim=0; dim < ND_ND; dim++){
            ...
            debug_displ[dim] = initial_coords[i+dim] + displacements[i+dim];
          }
          printf("Setting displacement for index[%d] = (%f, %f)\n",i,
                  debug_displ[0], debug_displ[1]);
          i += ND_ND;
        }
      }
    }
